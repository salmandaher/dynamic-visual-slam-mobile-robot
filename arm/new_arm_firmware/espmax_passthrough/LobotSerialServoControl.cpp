...
int LobotSerialServoControl::LobotSerialServoReceiveHandle(byte *ret)
{
  ...
  int _guard = 80;   // cap bytes processed: prevents an infinite loop (and a hung
                     // feedback timer) if the half-duplex line streams noise.
  while (SerialX->available() && _guard-- > 0) {
    rxBuf = SerialX->read();
  ...
  return -1;   // no complete/valid frame received (stock code fell off the end here)
}
...
int LobotSerialServoControl::LobotSerialServoReadPosition(uint8_t id)
{
  ...
  if(isAutoEnableRT == false)
    RxEnable();
  // Wait for the servo's reply with a TIME-based timeout. The stock library used a
  // fixed iteration counter (count=10000) tuned for a 16 MHz AVR; on the 240 MHz
  // ESP32 that loop expires far sooner than the servo's reply latency, so every
  // read returned -1. 20 ms is ample for a Lobot bus servo to answer.
  ...
  // Let the full reply frame (~8 bytes, ~0.7 ms at 115200) finish arriving before
  // parsing; otherwise ReceiveHandle often sees a partial frame -> checksum fail
  // (returned -2048). This is what made reads only ~40% reliable.
  delay(2);
